...
import { Mail, Lock, User, Zap, LogIn, UserPlus, ShieldCheck, Download, Smartphone } from 'lucide-react';
...
const AuthScreen: React.FC<AuthScreenProps> = ({ onAuth, appLogo, deferredPrompt, onInstall }) => {
        ...
        {deferredPrompt && (
          <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full">
            <button 
              ...
              className="w-full bg-blue-600/10 border border-blue-500/20 py-3 rounded-2xl flex items-center justify-center gap-3 text-blue-400 font-black text-xs shadow-lg active:scale-95 transition-all"
            >
              <Smartphone size={16} /> تثبيت التطبيق الأصلي (Android)
            </button>
          </motion.div>
        )}
